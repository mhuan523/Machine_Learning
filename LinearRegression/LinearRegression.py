import numpy as np
from LinearRegression.metrics import r2_score
...
class LinearRegression:
    ...
    def fit_gd(self, X_train, y_train, eta=0.01, n_iters=1e4):
        ...
        def gradient_descent(X_b, y, initial_theta, eta, n_iters=1e4, epsilon=1e-8):
            ...
            while cur_iter < n_iters:
                ...
                if (np.absolute(J(theta, X_b, y) - J(last_theta, X_b, y)) < epsilon):
                    break
                cur_iter += 1
    ...
    def fit_sgd(self, X_train, y_train, n_iter=5, t0=5, t1=50):
        ...
        X_b = np.hstack([np.ones(len(X_train), 1), X_train])
        initial_theta = np.zeros(X_b.shape[1])
        self._theta = sgd(X_b, y_train, initial_theta, n_iter, t0, t1)
    ...
    def predict(self, X_predict):
        """给定待预测数据集X_predict, 返回X_predict表示的结果向量"""
